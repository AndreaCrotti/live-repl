...
;; works when reset but not when just reloading
(defmacro mult-value [n]
  `(* 3 ~n))

(defn macro-driven [_request]
  {:status 200
   :body (str "Generated by macro: "
              (mult-value 10))})
...
(defn routes []
  #{["/health" :get `health-check :route-name :health]
    ...
    ["/macro-driven" :get `macro-driven :route-name :macro-driven]})
